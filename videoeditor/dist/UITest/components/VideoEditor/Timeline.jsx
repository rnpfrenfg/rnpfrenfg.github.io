...
export default function Timeline() {
    ...
    return (<div className="bg-gray-800 border-t border-gray-700 h-80 relative">
      <div className="flex h-full">
        <div className="w-48 bg-gray-900 border-r border-gray-700">
          ...
          <div className="space-y-1 p-2">
            {tracks.map((track) => (<div key={track.id} onClick={() => setSelectedTrack(track.id)} className={`p-2 rounded cursor-pointer transition-colors ${selectedTrack === track.id
                ? 'bg-blue-600'
        ...
        <div className="flex-1 overflow-x-auto">
          <div className="h-8 bg-gray-700 border-b border-gray-600 flex items-center px-4">
            <div className="flex space-x-8">
              {Array.from({ length: 10 }, (_, i) => (<div key={i} className="flex flex-col items-center">
          ...
          <div className="p-2 space-y-2">
            {tracks.map((track) => (<div key={track.id} className="h-12 bg-gray-700 rounded relative">
                <div className="flex items-center space-x-1 p-1">
                  {track.clips.map((clip) => (<div key={clip.id} onClick={() => handleClipClick(clip.id)} className={`h-10 ${clip.color} rounded px-2 flex items-center cursor-pointer hover:opacity-80 transition-all border-2 ${selectedClip === clip.id ? 'border-white' : 'border-transparent'}`} style={{ width: `${clip.duration * 4}px`, marginLeft: `${clip.start * 4}px` }}>
      ...
      {selectedClipDetails && (<div className="absolute bottom-0 right-0 w-80 bg-gray-900 border-l border-t border-gray-700 p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-white font-medium">Clip Properties</h4>
            <button onClick={() => setSelectedClip(null)} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white cursor-pointer">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="text" value={selectedClipDetails.name} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm mt-1" readOnly/>
            </div>
            
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-gray-400 text-sm">Start Time</label>
                <input type="text" value={`0:${Math.floor(selectedClipDetails.start / 60).toString().padStart(2, '0')}:${(selectedClipDetails.start % 60).toString().padStart(2, '0')}`} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm mt-1"/>
              </div>
              <div>
                <label className="text-gray-400 text-sm">Duration</label>
                <input type="text" value={`0:${Math.floor(selectedClipDetails.duration / 60).toString().padStart(2, '0')}:${(selectedClipDetails.duration % 60).toString().padStart(2, '0')}`} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm mt-1"/>
              </div>
            </div>
