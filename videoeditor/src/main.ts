...
function updateProgress(progress: number, stage?: "worker" | "encode") {
    const progressBar = document.getElementById('progress-bar') as HTMLDivElement;
    const progressText = document.getElementById('progress-text') as HTMLDivElement;
    const progressTitle = document.querySelector('#progress-modal h2') as HTMLHeadingElement | null;
    if (progressBar && progressText) {
        progressBar.style.width = `${progress}%`;
        const rounded = Math.round(progress);
        if (stage === "worker") {
            if (progressTitle) progressTitle.textContent = '합성/인코딩 중..';
            progressText.textContent = `합성 ${rounded}%`;
        } else if (stage === "encode") {
            if (progressTitle) progressTitle.textContent = '인코딩 중..';
